import Bean.HisTabBean;
import Bean.SqlTableBean;
...
public class Dao {
    ...
    public void CreataTab(String TableName) throws SQLException {
        String sql="create table "+TableName+" (ywname varchar(120),zwname varchar(120),danwei varchar(120),zdtype varchar(120),beizhu varchar(120),ispri varchar(120),crtime varchar(120),chtime varchar(120),user varchar(120))";
        Connection conn=Dbutil.getConnection();
    ...
    public void OutData(String TName){
        ...
        String sql="select *  into outfile 'D:/mysqlexport/"+OutName+".csv' fields terminated by ',' lines terminated by '\r\n' from(select '英文名称','中文名称','单位','字段类型','备注','主键' union select ywname,zwname,danwei,zdtype,beizhu,ispri from "+TName+")";
        Connection conn =Dbutil.getConnection();
        Statement st=null;
